...
export async function searchCashAppEmails(
  imap: Imap,
  sinceDate?: Date,
  allEmails: boolean = false
): Promise<number[]> {
  // If allEmails is true, search for all emails (not just Cash App)
  if (allEmails) {
    try {
      let criteria: any[] = ["ALL"]; // Search all emails
      ...
      return uids.sort((a, b) => b - a); // Sort newest first
    } catch (error) {
      return [];
    }
  }

  // IMAP search: Search for emails from Cash App senders
  // Note: Some IMAP servers don't support complex AND/OR operators
  // We'll search for each sender separately and combine results
  
  const senders = ["cash@square.com", "noreply@cash.app", "no-reply@cash.app"];
  const allUids: number[] = [];
  const seenUids = new Set<number>();

  // Search for each sender separately
  const searchPromises = senders.map(async (sender) => {
    try {
      let criteria: any[] = [["FROM", sender]];
  ...
  // Wait for all searches and combine unique UIDs
  const results = await Promise.all(searchPromises);
  for (const uids of results) {
  ...
  // Sort UIDs (newest first - higher UIDs are typically newer)
