...
export async function GET(
  ...
  try {
    ...
    const transaction = await Transaction.findOne({
      ...
      .populate("linkedDeliveryOrderIds", "restaurantName restaurantAddress appName miles money entryId userLatitude userLongitude userAddress additionalRestaurants")
      .lean();
    ...
    // Format dates as YYYY-MM-DD strings in Eastern Time
    const transactionObj: any = {
      _id: String(transaction._id),
      userId: transaction.userId,
      amount: transaction.amount,
      type: transaction.type,
      date: transaction.date ? formatDateAsET(new Date(transaction.date)) : "",
      time: transaction.time,
      isBill: transaction.isBill,
      isBalanceAdjustment: transaction.isBalanceAdjustment,
      notes: transaction.notes,
      tag: transaction.tag,
      dueDate: transaction.dueDate ? formatDateAsET(new Date(transaction.dueDate)) : undefined,
